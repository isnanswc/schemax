...
const LoreEntityItem: React.FC<{
  ...
  return (
    <div
      ...
      className="bg-slate-950/70 border border-slate-800/80 rounded-2xl p-3 space-y-2 cursor-pointer transition hover:border-slate-700/80 active:scale-[0.99]"
    >
      <div className="flex items-center gap-2.5">
        ...
        <div className="flex-1 min-w-0">
          ...
          <h5 className="font-bold text-xs sm:text-sm text-white truncate">{entity.name}</h5>
        </div>
        ...
        <div className="flex items-center gap-1">
          {onInsert && (
            <button
              ...
              className="py-1 px-2 rounded-lg bg-amber-500/15 hover:bg-amber-500/25 text-amber-300 border border-amber-500/30 text-[10px] font-bold transition active:scale-95"
      ...
      {isExpanded && (
        <div className="pt-2 border-t border-slate-800/80 space-y-2 animate-in fade-in">
          {entity.attributes && entity.attributes.length > 0 && (
            <div className="grid grid-cols-2 gap-1 text-[10px]">
              {entity.attributes.map((attr) => (
                <div key={attr.id} className="bg-slate-900 px-2 py-1 rounded-lg border border-slate-800 flex justify-between">
                  <span className="text-slate-500 truncate mr-1">{attr.label}</span>
                  <span className="font-medium text-slate-200 truncate">{attr.value}</span>
                </div>
              ))}
            </div>
          )}
          ...
          {entity.detailedNotes && (
            <p className="text-[10px] text-slate-300 italic bg-slate-900/60 p-2 rounded-lg border border-slate-800/60 leading-relaxed">
              {entity.detailedNotes}
            </p>
          )}
        </div>
...
export const LoreSidebarDrawer: React.FC<LoreSidebarDrawerProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden flex justify-end">
      ...
      <div className="relative w-full max-w-[320px] sm:max-w-[380px] bg-slate-900 border-l border-slate-800/90 h-full flex flex-col shadow-2xl z-10 animate-in slide-in-from-right duration-250 safe-top safe-bottom">
        {/* Drawer Header */}
        <div className="flex items-center justify-between px-4 py-3.5 border-b border-slate-800/80 bg-slate-950/60">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="font-bold text-sm text-white">Lore & Glosarium</h3>
              <p className="text-[10px] text-slate-400">{entities.length} entitas terdaftar</p>
            </div>
          </div>
        ...
        {/* Search Bar */}
        <div className="p-3 border-b border-slate-800/60 space-y-2 bg-slate-900">
