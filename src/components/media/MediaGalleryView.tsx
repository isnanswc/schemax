...
export const MediaGalleryView: React.FC<MediaGalleryViewProps> = ({
  ...
  return (
    <div className="space-y-4 pb-20">
      {/* Storage Banner */}
      <div className="bg-slate-900/90 border border-slate-800 rounded-2xl p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3 shadow-sm">
      ...
      {mediaList.length === 0 ? (
        <div className="text-center py-12 px-4 border border-dashed border-slate-800 rounded-2xl bg-slate-900/40">
          ...
          <h3 className="text-sm font-bold text-white mb-1">Galeri Visual Masih Kosong</h3>
          <p className="text-xs text-slate-400 max-w-sm mx-auto mb-4">
            Simpan gambar konsep, denah peta dunia, atau potret karakter ke dalam IndexedDB.
          </p>
          <label className="inline-flex items-center gap-2 py-2 px-4 bg-slate-800 hover:bg-slate-700 text-white font-semibold rounded-xl text-xs border border-slate-700 cursor-pointer transition">
      ...
      {selectedMedia && previewUrl && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85 backdrop-blur-md animate-in fade-in duration-200">
          <div className="relative max-w-2xl w-full bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-2xl animate-in zoom-in-95">
            {/* Top Bar */}
            <div className="flex items-center justify-between p-3.5 border-b border-slate-800 bg-slate-950/70">
              <div className="min-w-0 pr-3">
                <h4 className="text-sm font-bold text-white truncate">{selectedMedia.name}</h4>
            ...
            <div className="p-4 flex items-center justify-center max-h-[70vh] bg-slate-950/90 overflow-hidden">
              <img
                src={previewUrl}
            ...
            <div className="p-3 bg-slate-950/80 border-t border-slate-800 flex items-center justify-between">
              <a
                ...
                className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-xs font-semibold text-white transition"
              >
              ...
              <button
                ...
                className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-semibold text-red-400 hover:bg-red-500/10 transition"
              >
                <Trash2 className="w-3.5 h-3.5" />
