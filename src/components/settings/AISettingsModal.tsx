...
export const AISettingsModal: React.FC<AISettingsModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
      ...
      <div className="relative w-full sm:max-w-2xl bg-slate-900 border-t sm:border border-slate-800 rounded-t-3xl sm:rounded-3xl shadow-2xl z-10 max-h-[92vh] flex flex-col animate-in slide-in-from-bottom duration-250 safe-bottom">
        {/* Mobile Swipe Handle */}
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h3 className="font-extrabold text-base text-white">Pengaturan Multi-AI</h3>
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-5 space-y-4 no-scrollbar">
          ...
          <div className="bg-gradient-to-r from-amber-500/10 via-slate-900 to-indigo-500/10 border border-amber-500/20 rounded-2xl p-3.5 space-y-2">
            <div className="flex items-center justify-between gap-3">
              ...
              <button
                ...
                className={`relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${
                  config.smartAdjustEnabled ? 'bg-amber-500' : 'bg-slate-700'
                }`}
              >
                <span
          ...
          <div className="flex items-center gap-2 bg-slate-950/70 p-1 rounded-2xl border border-slate-800/80">
            <button
            ...
            >
              ...
              <span className="px-1.5 py-0.2 rounded-full text-[10px] font-black bg-slate-950/40 text-amber-950">
            ...
            >
              ...
              <span className="px-1.5 py-0.2 rounded-full text-[10px] font-black bg-slate-950/40 text-indigo-200">
          ...
          <div className="bg-slate-950 border border-slate-800 rounded-2xl p-4 space-y-3 shadow-md">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-800/80 pb-3">
              ...
              <button
                ...
                className="inline-flex items-center justify-center gap-1.5 py-1.5 px-3 rounded-xl bg-slate-800 hover:bg-slate-700 text-amber-300 font-bold text-xs transition active:scale-95 border border-slate-700 disabled:opacity-50 flex-shrink-0"
              >
                {isFetchingModels ? (
                  <>
            ...
            {fetchModelNotice && (
              <div className="p-2.5 bg-slate-900 border border-slate-800 rounded-xl text-xs text-slate-200 animate-in fade-in">
                {fetchModelNotice}
              </div>
            )}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5 pt-1">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full bg-slate-900 border border-slate-800 rounded-xl px-2.5 py-2 text-xs text-amber-300 font-bold focus:outline-none focus:border-amber-400"
                >
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full bg-slate-900 border border-slate-800 rounded-xl px-2.5 py-2 text-xs text-slate-200 focus:outline-none focus:border-amber-400"
                >
                  {currentGlobalConfig.cachedModels.map((m) => (
                    <option key={m.id} value={m.id}>
                      {m.name}
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full bg-slate-900 border border-slate-800 rounded-xl px-2.5 py-2 text-xs text-slate-300 focus:outline-none focus:border-amber-400"
                >
                  {currentGlobalConfig.cachedModels.map((m) => (
                    <option key={m.id} value={m.id}>
                      {m.name}
          ...
          <div className="space-y-3">
            ...
            {currentSlots.length === 0 ? (
              <div className="text-center py-8 px-4 border border-dashed border-slate-800 rounded-2xl bg-slate-950/40">
                ...
                <button
                  ...
                  className="inline-flex items-center gap-1.5 py-2 px-3.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-white text-xs font-semibold"
                >
              ...
              currentSlots.map((slot) => {
                ...
                return (
                  <div
                    key={slot.id}
                    className="bg-slate-950/90 border border-slate-800 rounded-2xl p-3.5 space-y-2.5 shadow-sm"
                  >
                    {/* Slot Name & Status */}
                    <div className="flex items-center justify-between gap-2">
                      <div className="flex items-center gap-2 min-w-0">
                        <input
                          ...
                          className="bg-transparent font-bold text-xs sm:text-sm text-white focus:outline-none focus:border-b border-amber-400 truncate max-w-[150px] sm:max-w-xs"
                    ...
                    <div className="relative">
                      <input
                        ...
                        className="w-full pl-3 pr-24 py-2 bg-slate-900 border border-slate-800 rounded-xl text-xs text-white placeholder-slate-600 focus:outline-none focus:border-amber-400 font-mono"
                      />
                      ...
                      <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-1.5">
                        ...
                        <button
                          ...
                          className="py-1 px-2 rounded-lg bg-slate-800 hover:bg-slate-700 text-amber-300 font-bold text-[10px] transition active:scale-95 disabled:opacity-40"
                        >
            ...
            <button
              type="button"
              onClick={() => handleAddSlot(activeTab)}
              className="w-full py-2.5 px-4 rounded-2xl bg-slate-950 border border-dashed border-slate-800 hover:border-slate-700 text-slate-300 hover:text-white font-semibold text-xs transition active:scale-[0.99] flex items-center justify-center gap-1.5"
            >
        ...
        <div className="p-4 border-t border-slate-800/80 bg-slate-950/60 flex items-center justify-between gap-3 flex-shrink-0">
