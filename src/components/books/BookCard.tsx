...
import { Calendar, FileText, ArrowRight, MoreVertical, Trash2, CheckCircle2, Edit3 } from 'lucide-react';
import { db } from '../../db';
...
export const BookCard: React.FC<BookCardProps> = ({ book, onSelect, chapterCount = 0 }) => {
  ...
  const formattedDate = new Date(book.updatedAt || book.createdAt).toLocaleDateString('id-ID', {
    day: 'numeric',
    month: 'short',
    year: 'numeric'
  });
  ...
  return (
    <div
      ...
      className="group relative bg-slate-900/90 hover:bg-slate-900 border border-slate-800/80 hover:border-slate-700/80 rounded-2xl p-3.5 transition-all duration-200 active:scale-[0.99] cursor-pointer flex gap-4 shadow-sm hover:shadow-lg hover:shadow-black/40"
    >
      ...
      <div className="flex-1 flex flex-col justify-between min-w-0">
        <div>
            ...
            <div className="relative">
              ...
              {showMenu && (
                <div 
                  ...
                  className="absolute right-0 top-6 w-44 bg-slate-950 border border-slate-800 rounded-xl shadow-xl z-20 py-1 text-xs animate-in fade-in zoom-in-95"
                >
                  <button
          ...
          {/* Book Title */}
          <h3 className="font-bold text-base text-white group-hover:text-amber-300 transition-colors line-clamp-1 leading-snug">
            {book.title}
          </h3>

          {/* Synopsis */}
          <p className="text-xs text-slate-400 line-clamp-2 mt-1 leading-relaxed">
            {book.synopsis || 'Belum ada sinopsis. Klik untuk mulai menulis cerita dan worldbuilding.'}
          </p>
        </div>
        ...
        <div className="pt-2 border-t border-slate-800/60 mt-2 flex items-center justify-between text-[11px] text-slate-400">
          <div className="flex items-center gap-3">
            ...
            <span className="hidden xs:inline-block text-slate-500">•</span>
